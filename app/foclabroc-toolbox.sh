...
check_internet() {
    if ! curl -s --head --connect-timeout 3 https://www.google.com >/dev/null; then
        dialog --backtitle "Foclabroc Toolbox" \
               --title "Erreur" \
               --msgbox "\nPas de connexion Internet détectée !" 6 40 2>&1 >/dev/tty
        exit 1
    fi
}
...
main_menu() {
    while true; do
        main_menu=$(dialog --clear --backtitle "Foclabroc Toolbox" \
            ...
            13 "[Tools]               -> Outils pour Batocera. Screenshot, Records..." \
            14 "[Install dans Ports]  -> Ajoute ce menu aux ports Batocera" \
            15 "[Underground]         -> !!!Mot de passe nécessaire !!!" \
            16 "[Exit]                -> Quitter le script" \
            2>&1 >/dev/tty)
        clear
