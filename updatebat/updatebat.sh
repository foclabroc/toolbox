...
# Fonction: télécharge avec annulation possible
telecharger_fichier() {
  ...
  (
    curl -L "$url" --progress-bar --output "$DEST_FILE" --fail &
    curl_pid=$!
...
# Fonction: mise à jour /boot
extraire_et_mettre_a_jour() {
  # Vérifie espace libre dans /boot
...
while true; do
  dialog --backtitle "$BACKTITLE" \
    --title "Confirmation" \
    --yesno "Version de Batocera actuelle : $VERSION\n\nÊtes-vous sûr de vouloir installer la version $numero_version ?" 9 60 2>&1 >/dev/tty
